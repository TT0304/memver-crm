  ...
  setup(props) {
    ...
    const submit = handleSubmit((values, { setFieldError }) => {
      ...
      form
        ...
        .post(permission ? route('permissions.update', permission.id) : route('permissions.store'), {
          onSuccess: (response) => {
...
<template>
  <Layout>
    <Head :title="$t('settings.permissions.create-title')" />
    <PageHeader :title="$t('settings.permissions.create-title')" :pageTitle="$t('settings.permissions.title')" />
            ...
            <BCard no-body>
              <BCardBody>
                <BCardHeader class="align-items-center d-flex border-bottom-0">
                  ...
                  <div v-if="$page.props.auth.user.permissions?.map((pr)=>pr.name).includes('permission_create')" class="flex-shrink-0">
                    <button type="submit" variant="primary" class="btn btn-primary">
                      <i class="ri-add-line align-bottom me-1"></i>
                      {{ $t("settings.permissions.save-btn-title") }}
                    </button>
                  </div>
