  ...
  setup(props) {
    ...
    const submit = handleSubmit((values, { setFieldError }) => {
      ...
      form
        ...
        .post((product ? route('products.update', product.id) : route('products.store')), {
          onSuccess: (response) => {
            ...
            <BCard no-body>
              <BCardBody>
                <BCardHeader class="align-items-center d-flex border-bottom-0">
                  ...
                  <div v-if="$page.props.auth.user.permissions?.map((pr)=>pr.name).includes('product_create')" class="flex-shrink-0">
